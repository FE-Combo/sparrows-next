...
export const middleware =
  (options?: Options) =>
  async (ctx: ParameterizedContext<DefaultState, ApiCTX>, next: Next) => {
      ...
      if (/\/api\/.*/.test(ctx.path)) {
        ...
        const matchWhitelistUrl =
          whitelist?.length > 0
            ...
            : undefined;
        const matchSaveSessionUrl =
          saveSessionApi?.length > 0
            ? match(saveSessionApi, { decode: decodeURIComponent })
            : undefined;
        const matchremoveSessionUrl =
          removeSessionApi?.length > 0
            ? match(removeSessionApi, { decode: decodeURIComponent })
            : undefined;
        ...
        if (matchWhitelistUrl && matchWhitelistUrl(ctx.path)) {
          if (matchSaveSessionUrl && matchSaveSessionUrl(ctx.path)) {
            ...
            await saveSession(
              crypto.encrypt(JSON.stringify({ test: "test2" })),
              ctx
            );
          } else {
...
function createExistMiddlewaresLogs(
  ctx: ParameterizedContext<DefaultState, ApiCTX>
) {
  if (ctx.jaeger) {
    if (ctx.csrf) {
...
async function getSession(ctx: ParameterizedContext<DefaultState, ApiCTX>) {
  ...
  const session = await redis.redis.get(
    redis?.sessionOptions?.prefix + redisKey
  );
