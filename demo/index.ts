...
import { middleware as redisMiddleware, KoaRedis } from "sparrows/dist/middlewares/redis";
import { middleware as corsMiddleware } from "sparrows/dist/middlewares/cors";
import { middleware as proxyMiddleware, Options as ProxyOptons} from "sparrows/dist/middlewares/proxy";
...
export const withDemo = (config: ApiConfig)=> {
    ...
    const nextMiddlewares = [proxyMiddleware(proxyOptions), bodyparser(), jaegerMiddleware(jaegerOptions), csrfMiddleware(csrfOptions), redisMiddleware({redisOptions, sessionOptions}), apiMiddleware(apiOptions)];
    ...
    const middlewares = [errorMiddleware(), corsMiddleware(), appMiddleware(appMiddlewareOptions)]
    return {
        middlewares,
        ...restConfig
    }
