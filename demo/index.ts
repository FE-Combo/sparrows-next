...
export const withDemo = (config: ApiConfig)=> {
    ...
    const appMiddlewares = [jaegerMiddleware(jaegerOptions), csrfMiddleware(csrfOptions), redisMiddleware({redisOptions, sessionOptions}), proxyMiddleware(proxyOptions)]
    return {
        assetPrefix,
