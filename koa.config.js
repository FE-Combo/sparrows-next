...
const apiOptions = {
  redisSecrets:
    "0011111111222222223333333344444444,01qqqqqqqqwwwwwwwweeeeeeeerrrrrrrr,02zzzzzzzzxxxxxxxxccccccccvvvvvvvv, 0300000100qqqqdqqqeeeseeeeyfyyyyyy",
  cookieSecret: "afeijflwekjflkjfifffjdfjkmdd",
  whitelist: [
    "/api/v1.0/industry/user/verify-login",
    "/api/v1.0/industry/user/verify-login2",
  ],
    ...
    set: function (sid, session) {
      ...
      if (cookieKey) {
        ...
        setTimeout(() => {
          this.sessionStore.destroy(
            signature.sign(cookieKey, apiOptions.cookieSecret)
          );
        }, 30000);
      }
      ...
      if (!this.res.headersSent) {
        this.cookies.set(
          sessionOptions.key,
          signature.unsign(
            sid.replace(/^s:koanext::$/, ""),
            apiOptions.cookieSecret
          ),
          session.cookie
        );
      }
